...
スキル発動から Step 6 完了まで、以下を順守する。これにより、ユーザーは Step 3 のヒアリングと Step 4 の Plan 承認だけで、あとは自走完了まで介入不要になる。

### AskUserQuestion / ユーザー待ちを許す場面

- Step 1.4 の CLAUDE.md 編集承認
- Step 3 のヒアリング 4 項目
- Step 4 の Plan 承認（ExitPlanMode）
- 進行不能で自力判断が許容範囲を超える分岐のみ（破壊的変更、外部依存の新規追加、ライセンス選択等）

**これ以外では聞かない**。「次に進む？」「これで OK？」「Phase X 終わったけど続ける？」「リスタートしますか？」系の確認は **禁止**。Phase 完了は即次の Phase へ移る。

### エラーと判断のデフォルト

- `read_console` error → 即修正を試みる、ユーザーに相談しない
- 同種エラー 2 回連続 → `./playbook/lessons/<topic>.md` を Read、対処、解決したら lessons に追記して進む
- スコープ追加の誘惑 → Plan で承認したスコープを死守、新機能は 80% 超で打ち切り（playbook §6）
- 既存シーン / Asset の破壊的変更 → ヒアリングで「既存継続」と答えていなければ、新規シーン / 新規ファイルを default にして既存には触らない
- フェーズ間の遷移 → 確認せず即次へ

### 時間規律

- 各フェーズ末で `/tmp/autopilot_start` 起点の経過時間を bash で計算してログ出力（Step 5 の経過時間計測コマンド）
- 80% 超過時点で新機能打ち切り、リファクタ・検証・仕上げに切り替え
- 早期完了見込み（経過 < 予算 × 0.5 かつ残機能が見えている）の場合、playbook §E の優先順位で残時間を使う：リファクタ深掘り → 動作確認 → polish → メモリ更新 → 新機能は最後の手段
...
## Step 2: セッション開始時刻記録（参考用）

```bash
date +%s > /tmp/autopilot_session_start
```

これはスキル発動 = ヒアリングと計画を含む **セッション全体** の参考時刻。
自走時間カウントの基準は **Step 4 末尾で記録する `/tmp/autopilot_start`**（計画承認直後）にする。
...
### Step 4 末尾: 自走開始時刻記録（必須）

ユーザー承認直後に必ず以下を実行:

```bash
date +%s > /tmp/autopilot_start
```

**ここから** ユーザーがヒアリングで指定した稼働時間（例: 60 分）をカウントする。
ヒアリングと計画に何分使ったかは無関係で、自走時間は予算通り確保される。
...
### 経過時間の計測
各フェーズ末で以下を実行し、自走開始からの経過秒数 / 予算秒数 を確認:

```bash
ELAPSED=$(( $(date +%s) - $(cat /tmp/autopilot_start) ))
BUDGET_SEC=$(( <ヒアリングで聞いた分数> * 60 ))
echo "elapsed: ${ELAPSED}s / ${BUDGET_SEC}s ($((100 * ELAPSED / BUDGET_SEC))%)"
```

### 主要規則
- **時間規律**: フェーズ単位は ±25% 許容、**自走全体は ±10% で着地**（厳密でなくていいが意識する）
- **自走の 60-70% 時点で専用リファクタフェーズ**（playbook §4、§C 全体を深掘り）。Step 3-e の小リファクタとは別物
- **自走の 80% 超過で新機能打ち切り**（playbook §6）
- 詰まったら `./playbook/lessons/<topic>.md` を Read して個別解決

ここでの「自走の X%」は `/tmp/autopilot_start` 起点（計画承認時点）で計算する。スキル発動からの経過時間ではない。
